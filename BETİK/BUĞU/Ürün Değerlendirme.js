...
function processpage(req, hlink, change, gaid, steamstore){
  ...
  if (req.title.indexOf('Site Error') === -1 && (!!steamstore || (!!ratingtest.length || !!ratingtest2.length || !!ratingtest3.length))){
    ...
    if (!!ratingtest.length){
      ...
      rating = ratingtest2.text()||ratingtest3.text()||"İnceleme yok.";
    }
    ...
    if (!!ratingtest.length){
      log("Yüklenen incelemeler " + hlink + dup + steamstore);
    } else if (!!ratingtest2.length){
      log("Şimdilik inceleme yok " + hlink + dup + steamstore);
    } else {
      log("İnceleme yok " + hlink + steamstore);
    }
  ...
  if(!!saved){
    if (expiretimeenabled && parseInt(Date.now()/1000)-expiretime >= saved.time){
      ...
      if (isgapage){
        ...
        if (GAratings){
            sidebarwide.append(
                '<h3 class="sidebar__heading">İncelemeler:</h3><ul class="sidebar__navigation"><li class="sidebar__navigation__item"><a class="sidebar__navigation__item__link" href=' + getreviewlink(code,saved) + ' rel="nofollow" target="_blank">' + geticon(cleanrating(rating)) + '<span style="text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">' + rating + '</span></a></li>' +
                ...
                ( rating.indexOf('%') !== -1 ? '<li class="sidebar__navigation__item"><a class="sidebar__navigation__item__link" href="https://steamdb.info/app/' + code + '/" rel="nofollow" target="_blank"><span class="steamdb"></span><div class="sidebar__navigation__item__name">BuğuVT</div><div class="sidebar__navigation__item__underline"></div><div class="sidebar__navigation__item__count">' + steamdb(rating) + '%</div></a></li>' : '') + 
                ( meta !== "" ? '<li class="sidebar__navigation__item"><a class="sidebar__navigation__item__link" href="http://www.metacritic.com/game/pc/' + meta[1] + '" rel="nofollow" target="_blank"><span class="meta"></span><div class="sidebar__navigation__item__name">Ürüneleştir</div><div class="sidebar__navigation__item__underline"></div><div class="sidebar__navigation__item__count">' + meta[0] + '/100</div></a></li>' : '') + '</ul>'
            );
        }
        if(!!saved.rdate){
            sidebarwide.append('<h3 class="sidebar__heading">Çıkış Günayı:</h3><ul class="sidebar__navigation"></ul>');
        ...
        if (!!GAfeatures && !!saved.features){
          if(GAfeatures === 2){
            sidebarwide.append('<h3 class="sidebar__heading">Özellikler:</h3><ul class="sidebar__navigation"></ul>');
        ...
        if(!!GAtags && !!saved.tags){
          if(GAtags === 2){
            sidebarwide.append('<h3 class="sidebar__heading">Etiketler:</h3><ul class="sidebar__navigation"></ul>');
