...
// @downloadURL  https://greasyfork.org/tr/users/434964-tengrikut1923
...
function factoryReset() {
    if (unsafeWindow.confirm("Tüm ölçüler ile önbelleğe alınmış verileri boşsanlamak istiyor musunuz?")) {
        const keys = GM_listValues();
        keys.forEach((key) => GM_deleteValue(key));
        console.log("Varsayılan ölçülere dönüldü!");
        unsafeWindow.alert("Varsayılan ölçülere dönüldü!");
    }
}
...
function displaySettings() {
    ...
    $("#title").text(`${name} (${version}) ${author}`);
    $("#settings").show();
    $("#notinstalled").hide();
...
function getIconHTML(color, str, lcs, icon, link) {
    ...
    const titlePlus = `\nSon Güncelleme: (${lcs}) \n${name} \nİye ${author}`;
    if (link) {
        return `<span title="${str}"><a style="color: ${color} !important;" href="${link}" target="_blank">${icon}</a></span>`;
    }

    return `<span style="color: ${color} !important;" title="${str}">${icon}</span>`;
}
    ...
    GM_xmlhttpRequest({
        ...
        "onload":  (response) => {
            ...
            try {
                ...
                console.log("Kaldırılan ürün verileri ayrıştırılamıyor. Önbelleğe alınmış veriler kullanılıyor...", error);
            }
            callback(cachedDecommissioned);
        },
        "onerror": () => {
            console.log("Kaldırılan ürün verileri yenilenirken bir yanılgı oluştu. Önbelleğe alınmış veriler kullanılıyor...");
            callback(cachedDecommissioned);
        },
        "ontimeout": () => {
            console.log("Kaldırılan ürün verilerini yenilemek çok uzun sürdü. Önbelleğe alınmış veriler kullanılıyor...");
            callback(cachedDecommissioned);
        },
    });
}
    ...
    GM_xmlhttpRequest({
        ...
        "onload":  (response) => {
            ...
            try {
                ...
                console.log("Barter.vg'nin indirilebilir içerik verileri ayrıştırılamıyor. Önbelleğe alınmış veriler kullanılıyor...", error);
            }
            callback(cachedDLC);
        },
        "onerror": (response) => {
            console.log("Barter.vg'nin indirilebilir içerik verileri yenilenirken bir yanılgı oluştu. Önbelleğe alınmış veriler kullanılıyor...", response);
            callback(cachedDLC);
        },
        "ontimeout": () => {
            console.log("Barter.vg'nin indirilebilir içerik verilerini yenilemek çok uzun sürdü. Önbelleğe alınmış veriler kullanılıyor...");
            callback(cachedDLC);
        },
    });
}
    ...
    GM_xmlhttpRequest({
        ...
        "onload":  (response) => {
            ...
            try {
                ...
                console.log("Barter.vg'nin güvenirliği düşük ölçümlü verileri ayrıştırılamıyor. Önbelleğe alınmış veriler kullanılıyor...", error);
            }
            callback(cachedLimited);
        },
        "onerror": (response) => {
            console.log("Barter.vg'nin güvenirliği düşük ölçümlü verileri yenilenirken bir yanılgı oluştu. Önbelleğe alınmış veriler kullanılıyor...", response);
            callback(cachedLimited);
        },
        "ontimeout": () => {
            console.log("Barter.vg'nin güvenirliği düşük ölçümlü verilerini yenilemek çok uzun sürdü. Önbelleğe alınmış veriler kullanılıyor...");
            callback(cachedLimited);
        },
    });
}
    ...
    GM_xmlhttpRequest({
        ...
        "onload":  (response) => {
            ...
            try {
                ...
                console.log("Barter.vg'nin yaprak verileri ayrıştırılamıyor. Önbelleğe alınmış veriler kullanılıyor...", error);
            }
            callback(cachedCards);
        },
        "onerror": (response) => {
            console.log("Barter.vg'nin yaprak verileri yenilenirken bir yanılgı oluştu. Önbelleğe alınmış veriler kullanılıyor...", response);
            callback(cachedCards);
        },
        "ontimeout": () => {
            console.log("Barter.vg'nin yaprak verilerini yenilemek çok uzun sürdü. Önbelleğe alınmış veriler kullanılıyor...");
            callback(cachedCards);
        },
    });
}
    ...
    GM_xmlhttpRequest({
        ...
        "onload":  (response) => {
            ...
            try {
                ...
                console.log("Barter.vg'nin tutam verileri ayrıştırılamıyor. Önbelleğe alınmış veriler kullanılıyor...", error);
            }
            callback(cachedBundles);
        },
        "onerror": (response) => {
            console.log("Barter.vg'nin tutam verilerini yenilerken bir yanılgı oluştu. Önbelleğe alınmış veriler kullanılıyor...", response);
            callback(cachedBundles);
        },
        "ontimeout": () => {
            console.log("Barter.vg'nin tutam verilerini yenilemek çok uzun sürdü. Önbelleğe alınmış veriler kullanılıyor...");
            callback(cachedBundles);
        },
    });
}
...
function doApp(elem, wishlist, ownedApps, ignoredApps, followedApps, decommissioned, limited, cards, bundles, dlc, lcs, dlcs, dlclcs, llcs, clcs, blcs) {
    ...
    setTimeout(() => { // avoids having the page hang when loading, because it is waiting on our script execution
        ...
        if (settings.wantDLC && dlc && dlc[appID]) { // if DLC and enabled
            ...
            html += getIconHTML(settings.dlcColor, `${subject} İndirilebilir içerik. ${ownsBase ? "" : ""}`, dlclcs, settings.dlcIcon + extraIcon); // ⇩
            iconsEncoding += 6;
        }
        ...
        if (today.includes("-04-01 ")) {
            html += getIconHTML("green", "1 Nisan! \nGüldürü için tıklayınız.", today, "&#129313;&#xFE0E;", "https://steamcommunity.com/id/TengriKut1923/");
        }
...
function integrate(userdata, decommissioned, cards, bundles, limited, dlc, lastCached) {
    ...
    $(document).ready(() => {
        ...
        GM_registerMenuCommand("Yine çalıştırınız.", () => doSWI(0));
        GM_registerMenuCommand("Tümünü arıklayınız.", clearSWI);
        GM_registerMenuCommand("Arıklayıp yine çalıştırınız.", reloadSWI);
    ...
    GM_xmlhttpRequest({
        ...
        "onload": (response) => {
            ...
            if (userdata.rgOwnedApps.length === 0 && userdata.rgOwnedPackages.length === 0) { // not logged in
                if (!cachedJson) {
                    console.log("Önbelleğe alınmış bilgi yok. Bunu düzeltmek için Buğu'ya giriş yapınız.");
                    return;
                }
...
function init() {
    ...
    GM_registerMenuCommand("Ölçüleri değiştiriniz.", () => unsafeWindow.open(settingsUrl, "_blank"));

    // Factory reset on any web page.
    GM_registerMenuCommand("Varsayılan ölçüler.", factoryReset);
}
